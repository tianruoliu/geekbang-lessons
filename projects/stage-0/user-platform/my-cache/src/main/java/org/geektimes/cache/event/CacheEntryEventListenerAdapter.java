...
public class CacheEntryEventListenerAdapter<K, V> implements ConditionalCacheEntryEventListener<K, V> {
    ...
    private static List<Object> eventTypesAndHandleMethodNames = asList(EventType.CREATED, "onCreated",
        EventType.UPDATED, "onUpdated", EventType.EXPIRED, "onExpired", EventType.REMOVED, "onRemoved");
    ...
    private Map<EventType, Method> determineEventTypeMethods(
        ...
        Class<? extends CacheEntryListener> cacheEntryListenerClass = cacheEntryListener.getClass();

        for (int i = 0; i < eventTypesAndHandleMethodNames.size(); i++) {
            EventType eventType         = (EventType)eventTypesAndHandleMethodNames.get(i++);
            String    handleMethodNames = (String)eventTypesAndHandleMethodNames.get(i++);

            Method handleMethod = null;
            try {
                handleMethod = cacheEntryListenerClass.getMethod(handleMethodNames, Iterable.class);
